  ...
  for (std::string libname : Library::list()) {
    ...
    if (lib.debug()) {
      lib.debugChannel().addTarget(
          new PrefixTarget(debug_prefix(libname), target));
    }
  }
}
...
FortranUnitTarget::FortranUnitTarget(int unit)
    : eckit::CallbackTarget(&write_to_fortran_unit, &unit_), unit_(unit) {}

LogTarget *createStyleTarget(LogTarget *target, Log::Style style,
                             const char *prefix) {
  if (style == Log::SIMPLE)
    return target;
  if (style == Log::PREFIX)
...
void Log::addFile(const std::string &path, Style style,
                  const std::string &prefix) {
  return addFile(path.c_str(), style, prefix.c_str());
}

void Log::setFile(const std::string &path, Style style,
                  const std::string &prefix) {
...
int Log::output_unit() { return fckit_fortranunit_stdout(); }
int Log::error_unit() { return fckit_fortranunit_stderr(); }
