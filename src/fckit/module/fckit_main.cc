...
void fckit__main_finalise() { fckit::Main::finalise(); }
...
int32 fckit__main_debug() { return fckit::Log::debug() != 0; }
...
int32 fckit__main_displayname(char *&name, size_t &size) {
  std::string v =
      dynamic_cast<const fckit::Main &>(fckit::Main::instance()).displayName();
  size = v.size();
  name = new char[size + 1];
...
void fckit__abort(const char *what, const char *file, int32 line,
                  const char *function) {
  fckit_abort(what, eckit::CodeLocation(file, line, function));
}

void fckit__exception_throw(const char *what, const char *file, int32 line,
                            const char *function) {
  fckit_throw(what, eckit::CodeLocation(file, line, function));
}

void fckit__set_signal_handler(int32 signum,
                               fckit::signal_handler_t signal_handler) {
  fckit::Signals::instance().setSignalHandler(
      fckit::Signal(signum, signal_handler));
}
...
void fckit__raise_signal(int32 signum) { std::raise(signum); }
