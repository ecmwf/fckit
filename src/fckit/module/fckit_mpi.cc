...
const Comm *fckit__mpi__comm_default() { return &eckit::mpi::comm(); }
...
void fckit__mpi__allreduce_int32(const Comm *comm, const int32 *in, int32 *out,
                                 size_t count, int32 operation) {
  if (comm)
    comm->allReduce(in, out, count, eckit::mpi::Operation::Code(operation));
  else
    eckit::mpi::comm().allReduce(in, out, count,
                                 eckit::mpi::Operation::Code(operation));
}

void fckit__mpi__allreduce_int64(const Comm *comm, const int64 *in, int64 *out,
                                 size_t count, int32 operation) {
  // TODO: Use int64 when ECKIT-349 is finished
  assert_int64_support(Here());
  if (comm)
    comm->allReduce((long *)in, (long *)out, count,
                    eckit::mpi::Operation::Code(operation));
  else
    eckit::mpi::comm().allReduce((long *)in, (long *)out, count,
                                 eckit::mpi::Operation::Code(operation));
}

void fckit__mpi__allreduce_real32(const Comm *comm, const float *in, float *out,
                                  size_t count, int32 operation) {
  if (comm)
    comm->allReduce(in, out, count, eckit::mpi::Operation::Code(operation));
  else
...
void fckit__mpi__allreduce_real64(const Comm *comm, const double *in,
                                  double *out, size_t count, int32 operation) {
  if (comm)
    comm->allReduce(in, out, count, eckit::mpi::Operation::Code(operation));
  else
    eckit::mpi::comm().allReduce(in, out, count,
...
void fckit__mpi__allreduce_inplace_int32(const Comm *comm, int32 *inout,
                                         size_t count, int32 operation) {
  if (comm)
    comm->allReduceInPlace(inout, count,
                           eckit::mpi::Operation::Code(operation));
  else
    eckit::mpi::comm().allReduceInPlace(inout, count,
                                        eckit::mpi::Operation::Code(operation));
}

void fckit__mpi__allreduce_inplace_int64(const Comm *comm, int64 *inout,
                                         size_t count, int32 operation) {
  // TODO: Use int64 when ECKIT-349 is finished
  assert_int64_support(Here());
  if (comm)
    comm->allReduceInPlace((long *)inout, count,
                           eckit::mpi::Operation::Code(operation));
  else
    eckit::mpi::comm().allReduceInPlace((long *)inout, count,
                                        eckit::mpi::Operation::Code(operation));
}

void fckit__mpi__allreduce_inplace_real32(const Comm *comm, float *inout,
                                          size_t count, int32 operation) {
  if (comm)
    comm->allReduceInPlace(inout, count,
                           eckit::mpi::Operation::Code(operation));
...
void fckit__mpi__allreduce_inplace_real64(const Comm *comm, double *inout,
                                          size_t count, int32 operation) {
  if (comm)
    comm->allReduceInPlace(inout, count,
                           eckit::mpi::Operation::Code(operation));
...
void fckit__mpi__broadcast_int32(const Comm *comm, int32 *buffer, size_t count,
                                 size_t root) {
  if (comm)
...
void fckit__mpi__broadcast_int64(const Comm *comm, int64 *buffer, size_t count,
                                 size_t root) {
  // TODO: Use int64 when ECKIT-349 is finished
  assert_int64_support(Here());
  if (comm)
...
void fckit__mpi__broadcast_real32(const Comm *comm, float *buffer, size_t count,
                                  size_t root) {
  if (comm)
    comm->broadcast(buffer, count, root);
  else
...
void fckit__mpi__broadcast_real64(const Comm *comm, double *buffer,
...
static eckit::CountedBuffer *extract_buffer(const eckit::SharedBuffer &cb) {
  eckit::CountedBuffer *buf =
      const_cast<eckit::SharedBuffer &>(cb).operator->();
  buf->attach();
  return buf;
}

eckit::CountedBuffer *
fckit__mpi__broadcast_file(const Comm *comm, const char *path, size_t root) {
  if (comm)
...
void fckit__mpi__send_int32(const Comm *comm, int32 *buffer, size_t count,
                            int32 dest, int32 tag) {
  if (comm)
...
void fckit__mpi__send_int64(const Comm *comm, int64 *buffer, size_t count,
                            int32 dest, int32 tag) {
  // TODO: Use int64 when ECKIT-349 is finished
  assert_int64_support(Here());
  if (comm)
...
void fckit__mpi__send_real32(const Comm *comm, float *buffer, size_t count,
                             int32 dest, int32 tag) {
  if (comm)
    comm->send(buffer, count, dest, tag);
  else
...
void fckit__mpi__send_real64(const Comm *comm, double *buffer, size_t count,
                             int32 dest, int32 tag) {
  if (comm)
    comm->send(buffer, count, dest, tag);
  else
...
void fckit__mpi__receive_int32(const Comm *comm, int32 *buffer, size_t count,
                               int32 source, int32 tag, int32 *status) {
...
void fckit__mpi__receive_int64(const Comm *comm, int64 *buffer, size_t count,
                               int32 source, int32 tag, int32 *status) {
  // TODO: Use int64 when ECKIT-349 is finished
  assert_int64_support(Here());
  eckit::mpi::Status _status;
...
void fckit__mpi__receive_real32(const Comm *comm, float *buffer, size_t count,
                                int32 source, int32 tag, int32 *status) {
  eckit::mpi::Status _status;
  if (comm)
    _status = comm->receive(buffer, count, source, tag);
...
void fckit__mpi__receive_real64(const Comm *comm, double *buffer, size_t count,
                                int32 source, int32 tag, int32 *status) {
  eckit::mpi::Status _status;
  if (comm)
    _status = comm->receive(buffer, count, source, tag);
...
int fckit__mpi__isend_int32(const Comm *comm, int32 *buffer, size_t count,
                            int32 dest, int32 tag) {
  if (comm)
...
int fckit__mpi__isend_int64(const Comm *comm, int64 *buffer, size_t count,
                            int32 dest, int32 tag) {
  // TODO: Use int64 when ECKIT-349 is finished
  assert_int64_support(Here());
  if (comm)
...
int fckit__mpi__isend_real32(const Comm *comm, float *buffer, size_t count,
                             int32 dest, int32 tag) {
  if (comm)
    return comm->iSend(buffer, count, dest, tag).request();
  else
...
int fckit__mpi__isend_real64(const Comm *comm, double *buffer, size_t count,
                             int32 dest, int32 tag) {
  if (comm)
    return comm->iSend(buffer, count, dest, tag).request();
  else
...
int fckit__mpi__ireceive_int32(const Comm *comm, int32 *buffer, size_t count,
                               int32 source, int32 tag) {
  if (comm)
...
int fckit__mpi__ireceive_int64(const Comm *comm, int64 *buffer, size_t count,
                               int32 source, int32 tag) {
  // TODO: Use int64 when ECKIT-349 is finished
  assert_int64_support(Here());
  if (comm)
    ...
    return eckit::mpi::comm()
        .iReceive((long *)buffer, count, source, tag)
        .request();
}

int fckit__mpi__ireceive_real32(const Comm *comm, float *buffer, size_t count,
                                int32 source, int32 tag) {
  if (comm)
    return comm->iReceive(buffer, count, source, tag).request();
  else
...
int fckit__mpi__ireceive_real64(const Comm *comm, double *buffer, size_t count,
                                int32 source, int32 tag) {
  if (comm)
    return comm->iReceive(buffer, count, source, tag).request();
  else
